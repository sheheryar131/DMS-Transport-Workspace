...
function incidentPage(){return `<div class="panel">${simpleTable(['Status','Date / Time','Type / Description','Location','Participant','Staff'],state.incidents.map(x=>`<tr><td>${badge(x.status)}</td><td>${x.incident_at?new Date(x.incident_at).toLocaleString('en-AU'):'—'}</td><td>${esc(x.description||'—')}</td><td>${esc(x.location||'—')}</td><td>${esc(x.participant_name||'—')}</td><td>${esc(x.staff_name||'—')}</td></tr>`))}</div>`}

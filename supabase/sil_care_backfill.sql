...
-- were inserted before the webhook correctly recognized this field.
-- Confirmed via real submission data: the "Full Name" / "Who is this" style
-- field across these DMS Care forms is internally named "typeA" (not
-- camelCase "fullName" as first guessed), and "SIL Location" is "typeA63".
...
set participant_name = coalesce(nullif(participant_name,''), (
  ...
  where lower(regexp_replace(k, '^q[0-9]+_', '')) = 'typea' limit 1))
where participant_name is null or participant_name = '';

update orientation_checklists
set sil_location = coalesce(nullif(sil_location,''), (
  select payload->>k from jsonb_object_keys(payload) as k
  where lower(regexp_replace(k, '^q[0-9]+_', '')) = 'typea63' limit 1))
...
set support_worker_name = coalesce(nullif(support_worker_name,''), (
  select payload->>k from jsonb_object_keys(payload) as k
  where lower(regexp_replace(k, '^q[0-9]+_', '')) = 'nameof' limit 1))
where support_worker_name is null or support_worker_name = '';

update sil_visitor_checkins
set reason_for_visit = coalesce(nullif(reason_for_visit,''), (
  select payload->>k from jsonb_object_keys(payload) as k
  where lower(regexp_replace(k, '^q[0-9]+_', '')) = 'reasonfor' limit 1))
where reason_for_visit is null or reason_for_visit = '';

update sil_visitor_checkins
set duration = coalesce(nullif(duration,''), (
  select payload->>k from jsonb_object_keys(payload) as k
  where lower(regexp_replace(k, '^q[0-9]+_', '')) = 'estimatedduration' limit 1))
